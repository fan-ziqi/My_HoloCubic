...
void display_init(void)
{
  ...
  C_label = lv_label_create(scr[0], NULL);
...
void display_weather(const char *cityname, const char *temperature, int weathercode)
{
  ...
  lv_obj_align(weather_image, NULL, LV_ALIGN_OUT_TOP_MID, 0, 150);

  lv_obj_add_style(cityname_label, LV_LABEL_PART_MAIN, &label_style1);
  lv_label_set_text(cityname_label, cityname);
  lv_obj_align(cityname_label, NULL, LV_ALIGN_OUT_BOTTOM_LEFT, 20, -60);
  ...
  lv_obj_add_style(C_label, LV_LABEL_PART_MAIN, &label_style3);
  lv_label_set_text(C_label, "°C");
  lv_obj_align(C_label, temperature_label, LV_ALIGN_OUT_RIGHT_MID, 10, 0);
}

void display_time(const char *date, const char *time)
{
  // lv_img_set_src(time_image, &huoj);
  lv_img_set_src(time_image, &logo);
  lv_obj_align(time_image, NULL, LV_ALIGN_OUT_TOP_MID, 0, 120);

  lv_obj_add_style(date_label, LV_LABEL_PART_MAIN, &label_style5);
  lv_label_set_text(date_label, date);
  lv_obj_align(date_label, NULL, LV_ALIGN_OUT_BOTTOM_LEFT, 15, -100);
...
void display_hardware(const char *info)
{
  ...
  lv_label_set_text_fmt(cpu_used_label, "CPU Used: %d\%", 0);
